...
function usage {
    cat << EOF
Create archive of this project
...
EXAMPLES:
    ...
    2. $ME --outdir=/local/project
       creates an archive and stores it under /local/project

    3. $ME v3.0.0
       creates an archive /tmp/${PROJECT}-3.0.0.tar.bz2
EOF
}
...
# Remove "v" prefix:
VERSION=${VERSION#v*}
FILE="${OUTDIR}/${PROJECT}-${VERSION}${SUFFIX}"
